...
namespace gaussian_splatting
{
	...
	*/
	...
	public:
		...
		Eigen::Matrix4f		matrix(void) const;
		/// \return the inverse of the transformation matrix
		Eigen::Matrix4f		invMatrix(void) const;
		...
		static Transform3<T>	extrapolate(const Transform3<T>& previous, const Transform3<T>& current, float dist01) {
			...
			Vector3 deltaPosition = current.position() - previous.position();
	...
	template <typename T>
	Eigen::Matrix4f Transform3<T>::matrix(void) const {
		Eigen::Matrix4f trans = matFromQuat(_rotation);
		trans = matFromTranslation(_position) * trans; // Opti (direct)
		return trans;
	}

	template <typename T>
	Eigen::Matrix4f Transform3<T>::invMatrix(void) const {
